...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # create a matrix of shape (seq_len, d_moel)
        pe = torch.zeros(seq_len, d_model)
        posision = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1)

        div_term = torch.exp(-torch.arange(0, d_model, 2) / d_model * math.log(10000.0))

        pe[:, 0::2] = torch.sin(posision * div_term)
        pe[:, 1::2] = torch.cos(posision * div_term)
...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: InputEmbeddings, tgt_embed: InputEmbeddings, src_pos: PositionalEncoding, tgt_pos: PositionalEncoding, project_layer: ProjectionLayer) -> None:
        super().__init__()
